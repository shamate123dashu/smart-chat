...
//    @Select("select * from message where nameA=#{nameA} and nameB=#{nameB} and status=#{status} and messType=#{messType}")
    ...
    @Update("update message set status=1 where nameA=#{nameA} and nameB=#{nameB} and messType=#{messType}")
    ...
    //查找所有未读的消息
    @Select("select * from message where nameB=#{username} and status=#{status} and messType=#{messType} order by time desc")
